...
# _, mask = cv2.threshold(im[:, :, 3], 200, 240, cv2.THRESH_BINARY_INV) 
...
#         #waitKey(1) returns the key pressed in an integer form and ord('q') converts the character q to the
#         #integer form of q, so when q is pressed, it becomes the int.
#         break

# cv2.destroyAllWindows()

# cap = cv2.VideoCapture(0) #initialized the camera capture objecdt

 #reading the camera input and stores in frame and runs while ret is true

# _, frame = cap.read() #cap.read returns a single frame, so we need the while loop to get a video
# gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
# cv2.imwrite(r"image-thresholding/Webcam.png", gray)
# gray = cv2.imread(r"image-thresholding\Webcam.png")

# _, threshold = cv2.threshold(gray, 110, 255, cv2.THRESH_BINARY)
# cv2.imshow("Webcam", threshold)
'''
127 threshold: basically splits the image. If you have a light image, you would want a higher threshold to split
the colors. Try to find a middleground in the image between the background and the object you want to 
differentiate
'''


'''Live webcam threshold'''
# cap = cv2.VideoCapture(0) #initialized the camera capture objecdt

# while True: #while frame is capturing(ret is true)
#     #why is it returning a single frame
#     #how to get it to return the video stream
#     _, frame = cap.read() #cap.read returns a single frame, so we need the while loop to get a video

#     #convert the frame to grayscale before showing it: 
#     gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
...
'''BEGINNING
converting image to HSV (hue, saturation, value), then converting to gray to make it better'''
cap = cv2.VideoCapture(0) #initialized the camera capture objecdt
...
'''BEGINNING
converting threshold to different colors
'''
...
#     gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
#     b, g, r = cv2.split(frame) #gives grayscale of the color chanels
#     b = b*0
#     g = g*1
#     r = r*0
#     #adding the alpha to get initensity

#     image = cv2.merge([b, g, r])

#     _, colorThreshold = cv2.threshold(image, 127, 255, cv2.THRESH_BINARY)

#     cv2.imshow("Webcam", colorThreshold)
#     if cv2.waitKey(1) == ord('q'):
#         break
...
'''BEGINNING
TRYING TO PUT A BOX AROUND OBJ'''
# cap = cv2.VideoCapture(0) #initialized the camera capture objecdt
...
#     #now threshold the image: 
#     _, threshold = cv2.threshold(gray, 80, 255, cv2.THRESH_BINARY)
#     lineDetection = cv2.Canny(threshold, 127, 127, apertureSize=7, L2gradient=True)


#     cv2.imshow("Webcam", lineDetection)
